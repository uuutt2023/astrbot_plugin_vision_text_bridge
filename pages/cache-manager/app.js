...
(async function main() {
 ...
 if (badge) {
 badge.textContent = "bridge 模式 (quart)";
...
function initThemeToggle() {
 ...
 try { localStorage.setItem("vtb-theme", theme); } catch (e) {}
 // 同步 status chip 背景 (顶部徽章在切换时要重新对色)
 logger.info("theme", `切换主题: ${theme}`);
 };
 // 初始按钮文字反映当前主题
 const current = document.documentElement.getAttribute("data-theme") || "dark";
 btn.textContent = current === "light" ? "深色" : "浅色";
 btn.title = current === "light" ? "切换到深色主题" : "切换到浅色主题";
...
function renderList(items) {
 ...
 <div class= "empty-state-text">${state.search ? "无搜索结果" : "暂无缓存数据"}</div>
 ...
 <button class= "btn"data-action= "view"data-id= "${escapeHtml(it.image_id)}"title= "查看大图">查看</button>
 <button class= "btn"data-action= "regen"data-id= "${escapeHtml(it.image_id)}"title= "重新生成">重生成</button>
 <button class= "btn danger"data-action= "delete"data-id= "${escapeHtml(it.image_id)}"title= "删除">删除</button>
 </td>
 </tr>
 `;
 })
...
async function ensureThumb(imageId, slot) {
 ...
 if (cached.__err) {
 slot.innerHTML = `<div class= "thumb-placeholder"title= "缩略图加载失败"></div>`;
...
async function onClear() {
 ...
 if (!await customConfirm("确定要清空所有缓存条目吗？此操作不可撤销。", "清空确认")) {
...
async function onDiag() {
 ...
 <div class= "field-value"style= "color: var(--danger);"> SQLite 表是空的——没数据被写入</div></div>`;
 }
 }
 body.innerHTML = html;
 $("detail-modal").hidden = false;
